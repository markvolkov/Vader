...
fn run_simplest_server() {
    ...
    let test_get_route = Route::new("/test", "GET", |_req, mut res| {
    ...
    let test_post_route = Route::new("/test", "POST", |_req, mut res| {
        println!("{}", _req.body);
        res.with_status(StatusCode::Ok).write_html(_req.body.as_bytes());
    });
    some_server.map_route(&test_get_route);
    some_server.map_route(&test_post_route);
    some_server.start();
}
